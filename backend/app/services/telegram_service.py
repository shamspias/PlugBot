...
class TelegramService:
    ...
    def _escape_all_markdown(self, text: str) -> str:
        """
    ...
    def _validate_markdown(self, text: str) -> bool:
        ...
        Returns True if markdown is valid, False otherwise.
        """
        if not text:
            return True
        ...
        try:
            ...
            # Count underscores that are not escaped
            italic_count = 0
            i = 0
            while i < len(text):
            ...
            # If any validation fails, consider it invalid
            return False
    ...
    def _escape_markdown_safely(self, text: str) -> str:
        ...
        if not self._validate_markdown(sanitized):
            return self._escape_all_markdown(text)
    ...
    def _fmt(self, text: str, *, finalize: bool = False) -> dict:
        """
        Improved formatting helper with robust markdown support.
        """
        ...
        if finalize:
            try:
                ...
                if self._validate_markdown(safe_text):
                    escaped_text = self._escape_markdown_safely(safe_text)
                    return {
                        "text": escaped_text,
                    ...
                    return {
                        "text": self._escape_all_markdown(safe_text),
                        "parse_mode": ParseMode.MARKDOWN_V2,
                    }
    ...
    async def _send_message_safely(self, update, context, chat_id, message_id, text, is_edit=False):
        """
        Safely send or edit a message with fallback handling for markdown errors.
        """
        try:
            fmt_result = self._fmt(text, finalize=True)
        ...
        try:
            async for event in self.dify_service.send_message(
                ...
                if event.get("event") == "message":
                    ...
                    if self.bot.response_mode == "streaming":
                        if not message_id:
                            msg = await self._send_message_safely(update, context, chat_id, None, response_text,
                                                                  is_edit=False)
                            if msg:
                                message_id = msg.message_id
                            last_sent_text = response_text or ""
                        elif len(response_text) % 20 == 0 and response_text != last_sent_text:
                            await self._send_message_safely(update, context, chat_id, message_id, response_text,
                                                            is_edit=True)
            ...
            if response_text:
                if self.bot.response_mode == "blocking" or not message_id:
                    await self._send_message_safely(update, context, chat_id, None, response_text, is_edit=False)
                else:
